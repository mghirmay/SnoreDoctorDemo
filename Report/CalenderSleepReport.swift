...
struct CalenderSleepReport: View {
    ...
    /// Fetched once — all other properties derive from this.
    ...
    private var allSnoreEvents: [SnoreEvent] {
        sessions.flatMap { soundDataManager.fetchSnoreEvents(for: $0) }
    }

    private var allSoundEvents: [SoundEvent] {
        sessions.flatMap { soundDataManager.fetchSoundEvents(for: $0) }
    }
            ...
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    DailySleepReportView(
                        selectedDate: selectedDate,
                        sessions: sessions,
                        allSnoreEvents: allSnoreEvents,
                        allSoundEvents: allSoundEvents
                    )
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                }
                .padding()
